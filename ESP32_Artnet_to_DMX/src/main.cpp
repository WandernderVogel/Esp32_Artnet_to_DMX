/**
 * @ Name: Artent to DMX
 * @ Description: Artnet to DMX converter
 * @ Author: Wandernder Vogel
 * @ Create Time: 2025-09-15 12:08
 * @ Modified by: Wandernder Vogel
 * @ Modified time: 2025-09-15 20:53
 */
...
// ========================================================
// user defined vars PLEASE CHANGE BEFORE RUNNING

const char *ssid = "Your_WiFi_SSID";
const char *pwd = "Your_WiFi_PWD";

const int max_universes = 2;
...
const int tx2_pin = 19;
const int rx2_pin = 21; // not used, can be used if code is altered and device should be used for DMX-> Artnet

uint8_t dmx_data[max_universes][512] = {{0}, {0}};
EasyDMX dmx;

SoftwareSerial Serial3(rx2_pin, tx2_pin);
EasyDMX dmx2;
...
void setup()
{
  ...
  dmx2.begin(DMXMode::Transmit, DMXPin::NoRx, tx2_pin);
}
...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t *data)
{
  ...
  for (int i = 0; i < length; i++)
  {
    ...
    // Serial.print(" ");
    if (dmx_data[universe][i] != data[i])
    {
      (universe == startingUniverse) ? dmx.setChannel(i + 1, data[i]) : dmx2.setChannel(i + 1, data[i]);
    }
    dmx_data[universe][i] = data[i];
  }
  // Serial.println()
